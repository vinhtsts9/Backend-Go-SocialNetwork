...
func (q *Queries) GetFollowersByUserId(ctx context.Context, followingID sql.NullInt64) ([]GetFollowersByUserIdRow, error) {
	rows, err := q.db.QueryContext(ctx, getFollowersByUserId, followingID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
