...
// versions:
//   sqlc v1.29.0
// source: 0003_user_info_9999.sql
...
const updateAvatar = `-- name: UpdateAvatar :execresult
UPDATE ` + "`" + `user_info` + "`" + `
SET user_avatar = ?
WHERE user_id = ? AND user_is_authencation = 1
`
...
func (q *Queries) UpdateAvatar(ctx context.Context, arg UpdateAvatarParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateAvatar, arg.UserAvatar, arg.UserID)
}
