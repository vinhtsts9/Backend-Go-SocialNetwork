...
type ConnectionManager struct {
	...
	roomUsers  map[int][]*Client           // roomID -> danh sách Client
	broadcast  chan RoomMessage            // Kênh phát tin nhắn
	register   chan *Client                // Kênh đăng ký Client
	unregister chan *websocket.Conn        // Kênh hủy đăng ký Client
	mu         sync.RWMutex                // Đồng bộ hóa
}
...
func (cm *ConnectionManager) handleMessage(message []byte, client *Client, ctx *gin.Context) {
	...
	case "auth":
		...
		userInfo, err := auth.CheckAuthForWebsocket(tokenString) // Xử lý token
		if err != nil {
			client.Conn.WriteMessage(websocket.TextMessage, []byte("Authentication fail"))
			return
		}

		global.Logger.Sugar().Infof("User authenticated")
	...
	case "send_message":
		...
		chatMessage := model.ModelChat{
			...
			RoomId:         sql.NullInt64{Int64: int64(roomID), Valid: true},
